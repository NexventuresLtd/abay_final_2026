...
import {
  Search, Plus, Minus, Trash2, ShoppingCart, Receipt, Package
} from 'lucide-react';
...
export default function SalesPage() {
  ...
  // Mobile: toggle between product grid and cart panel
  const [mobileView, setMobileView] = useState<'products' | 'cart'>('products');

  const { data: productsData } = useQuery({
    queryKey: ['products-pos', search],
    queryFn: () => productsApi.list({ search, per_page: 24, stock_status: 'in' }).then(r => r.data),
    enabled: tab === 'pos',
  });

  const { data: salesData } = useQuery({
    queryKey: ['sales', historyPage],
    queryFn: () => salesApi.list({ page: historyPage, per_page: 20 }).then(r => r.data),
    enabled: tab === 'history',
  });
  ...
  const updateQty = (productId: number, qty: number) => {
    ...
    if (item && qty > item.product.quantity) { toast.error('Insufficient stock'); return; }
  ...
  const CartPanel = () => (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto space-y-0 min-h-0">
        {cart.length === 0 ? (
          <div className="py-10 text-center" style={{ color: 'var(--color-text-faint)' }}>
          ...
          cart.map(item => (
            <div key={item.product.id}
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-blue-600">{formatCurrency(item.unit_price)}</div>
              </div>
              ...
              <div className="text-xs font-semibold w-14 text-right shrink-0"
      ...
      {/* Totals + checkout */}
      <div className="shrink-0 pt-3 space-y-3 border-t mt-2"
        style={{ borderColor: 'var(--color-border)' }}>
        <div className="flex justify-between text-sm">
          <span style={{ color: 'var(--color-text-muted)' }}>Subtotal</span>
          <span style={{ color: 'var(--color-text)' }}>{formatCurrency(subtotal)}</span>
        </div>
        <div className="flex items-center justify-between gap-2">
          <span className="text-sm shrink-0" style={{ color: 'var(--color-text-muted)' }}>Discount ($)</span>
          <input
            type="number" value={discount || ''} placeholder="0"
        ...
        <div className="flex gap-1.5">
          {['cash', 'card', 'mobile'].map(m => (
            <button key={m} onClick={() => setPaymentMethod(m)}
              className={`flex-1 btn btn-sm capitalize ${paymentMethod === m ? 'btn-primary' : 'btn-secondary'}`}>
              {m}
            </button>
          ))}
        </div>
        ...
        <button
          ...
          disabled={saleMutation.isPending || !cart.length}
  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex gap-1 p-1 rounded-xl" style={{ background: 'var(--color-surface-3)' }}>
      ...
      {tab === 'pos' && (
        <>
          {/* Mobile view toggle */}
          <div className="flex lg:hidden gap-1 p-1 rounded-xl w-fit"
            style={{ background: 'var(--color-surface-3)' }}>
            <button onClick={() => setMobileView('products')}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                mobileView === 'products' ? 'bg-white dark:bg-gray-700 shadow-sm text-blue-700' : ''
              }`}
              style={{ color: mobileView === 'products' ? undefined : 'var(--color-text-muted)' }}>
              Products
            </button>
            <button onClick={() => setMobileView('cart')}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all flex items-center gap-1.5 ${
                mobileView === 'cart' ? 'bg-white dark:bg-gray-700 shadow-sm text-blue-700' : ''
              }`}
              style={{ color: mobileView === 'cart' ? undefined : 'var(--color-text-muted)' }}>
              Cart
              {cart.length > 0 && (
          ...
          <div className="flex flex-col lg:grid lg:grid-cols-5 gap-4">
            {/* Products grid */}
            <div className={`lg:col-span-3 space-y-3 ${mobileView === 'cart' ? 'hidden lg:block' : ''}`}>
              ...
              {/* Responsive product cards: 2 cols mobile, 3 on sm+ */}
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3">
                {productsData?.items?.map((product: Product) => (
                  <button key={product.id} onClick={() => { addToCart(product); setMobileView('cart'); }}
            ...
            {/* Cart panel */}
            <div className={`lg:col-span-2 ${mobileView === 'products' ? 'hidden lg:block' : ''}`}>
              <div className="card lg:sticky lg:top-4" style={{ minHeight: '500px' }}>
                <CartPanel />
              </div>
            </div>
          </div>
                  ...
                  ? salesData.items.map((sale: Sale) => (
                      <tr key={sale.id}>
                        <td>
                          ...
                          <div className="text-xs md:hidden" style={{ color: 'var(--color-text-muted)' }}>
                        ...
                        <td className="hidden sm:table-cell">
                          <span className="badge badge-blue capitalize">{sale.payment_method}</span>
                        </td>
                        <td>
          ...
          {salesData && salesData.pages > 1 && (
            <div className="flex items-center justify-between px-4 py-3 border-t"
              ...
              <div className="flex gap-1.5">
                ...
                <button onClick={() => setHistoryPage(p => Math.min(salesData.pages, p + 1))}
                  disabled={historyPage === salesData.pages} className="btn btn-secondary btn-sm">Next →</button>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          ...
          <div className="relative w-full sm:max-w-sm rounded-t-2xl sm:rounded-2xl overflow-hidden"
            ...
            <div className="p-6 text-center border-b" style={{ borderColor: 'var(--color-border)' }}>
              ...
              <h3 className="font-bold text-lg" style={{ color: 'var(--color-text)' }}>Sale Completed!</h3>
              <p className="text-sm font-mono" style={{ color: 'var(--color-text-muted)' }}>{receipt.reference}</p>
            </div>
            <div className="p-4 space-y-2 max-h-64 overflow-y-auto">
              {receipt.items.map(item => (
                <div key={item.id} className="flex justify-between text-sm">
                  ...
                  <span style={{ color: 'var(--color-text)' }}>{formatCurrency(item.total)}</span>
                </div>
              ))}
              <div className="border-t pt-2 space-y-1.5" style={{ borderColor: 'var(--color-border)' }}>
                ...
                <div className="flex justify-between font-bold text-base">
                  <span style={{ color: 'var(--color-text)' }}>Total</span>
                  <span className="text-blue-600">{formatCurrency(receipt.total_amount)}</span>
                </div>
                <div className="flex justify-between text-sm text-green-600">
                  <span>Change</span>
                  <span className="font-semibold">{formatCurrency(receipt.change_amount)}</span>
                </div>
              </div>
            </div>
            <div className="p-4 border-t flex gap-2" style={{ borderColor: 'var(--color-border)' }}>
              <button onClick={() => setReceipt(null)} className="btn btn-primary flex-1 justify-center">
                Start New Sale
              </button>
            </div>
          </div>
