...
import { z } from 'zod';
import { Plus, Search, Edit2, Trash2, Package, Filter, X, SlidersHorizontal } from 'lucide-react';
...
function ProductForm({
  ...
  const { register, handleSubmit, setValue, watch, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: product
      ...
      : { quantity: 0, low_stock_threshold: 10, cost_price: 0, selling_price: 0 },
  });
    ...
    <form onSubmit={handleSubmit(d => mutation.mutate(d))} className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="sm:col-span-2">
          ...
          <input {...register('name')} className="input" placeholder="e.g. Wireless Earbuds" />
          {errors.name && <p className="text-red-500 text-xs mt-1">{errors.name.message}</p>}
        </div>
        <div>
          <div className="flex items-center justify-between">
            <label className="label">SKU *</label>
            <button type="button" onClick={() => setValue('sku', generateSKU(watchedName || 'PRD'))}
              className="text-xs text-blue-600 hover:underline mb-1.5">Auto-generate</button>
          </div>
          <input {...register('sku')} className="input" placeholder="e.g. EAR-WL-001" />
          {errors.sku && <p className="text-red-500 text-xs mt-1">{errors.sku.message}</p>}
        </div>
        <div>
          <label className="label">Brand</label>
          <input {...register('brand')} className="input" placeholder="e.g. Sony" />
        </div>
        <div>
        ...
        <div>
          ...
          <input {...register('selling_price')} type="number" step="0.01" className="input" placeholder="0.00" />
        </div>
        <div>
...
export default function ProductsPage() {
  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      ...
      <div className="space-y-2">
        ...
        {filtersOpen && (
          <div className="card p-3 flex flex-wrap gap-2">
            ...
            {hasFilters && (
              <button
                onClick={() => { setCategoryId(''); setStockStatus(''); }}
            ...
            <tbody>
              {isLoading
                ? Array.from({ length: 6 }).map((_, i) => (
                    <tr key={i}>
                      {[80, 60, 70, 60, 40, 50, 40].map((w, j) => (
                        <td key={j}><div className={`skeleton h-4 w-${w === 80 ? '20' : w === 60 ? '16' : w === 40 ? '10' : '12'}`} /></td>
                      ))}
                    </tr>
                  ))
                : data?.items?.length
                      ...
                      return (
                        <tr key={product.id}>
                          ...
                          <td>
                            ...
                            <div className="text-xs hidden sm:block" style={{ color: 'var(--color-text-muted)' }}>
      ...
      {/* Create Modal */}
      <Modal open={modal === 'create'} onClose={closeModal} title="Add New Product">
      ...
      <Modal open={modal === 'edit'} onClose={closeModal} title="Edit Product">
        {selected && (
          <ProductForm product={selected} onSuccess={closeModal} onCancel={closeModal} />
        )}
      </Modal>
    </div>
  );
