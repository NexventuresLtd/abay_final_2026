...
import {
  ...
  BarChart3, DollarSign, Settings, LogOut, Menu, X,
  Moon, Sun, ChevronRight, Zap, Tag
} from 'lucide-react';
...
function Logo() {
  return (
    <div className="flex items-center gap-2 min-w-0">
      ...
      <span className="font-extrabold text-sm tracking-tight" style={{ color: 'var(--color-text)' }}>
      ...
      >
        ...
        <div className="shrink-0 p-3 border-t space-y-2"
          ...
          <div className="flex gap-1.5">
            <button onClick={toggleTheme} className="btn btn-secondary btn-sm flex-1 justify-center">
              {theme === 'dark' ? <Sun className="w-3.5 h-3.5" /> : <Moon className="w-3.5 h-3.5" />}
            </button>
            <button onClick={handleLogout} className="btn btn-danger btn-sm flex-1 justify-center">
      ...
      <div className="flex-1 flex flex-col min-h-screen lg:ml-60 min-w-0">
        ...
        >
          <button onClick={() => setDrawerOpen(true)}
            className="p-2 -ml-1 rounded-lg" style={{ color: 'var(--color-text)' }}>
            <Menu className="w-5 h-5" />
          </button>
          <Logo />
          <button onClick={toggleTheme}
            className="p-2 -mr-1 rounded-lg" style={{ color: 'var(--color-text-muted)' }}>
            {theme === 'dark' ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
          </button>
        </header>

        {/* pb-24 = space above mobile bottom nav (h-16 nav + safe area) */}
